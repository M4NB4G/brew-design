...
| Ingredient list in the app | The app carries a copy of the owner's ingredient workbook (`data/Brew Design Ingredients.xlsx`): malts (FGDB, colour), hops (alpha acid), yeasts (ale/lager, attenuation, lab temperature range), each number exactly its workbook cell; a test fails whenever copy and workbook disagree or the workbook holds a row the app could not use; nothing reads the copy yet. Routine workbook edits become Tier D. Scope table agreed: `docs/items/ingredient-list-in-app.md` | suites + inspector + far end |
