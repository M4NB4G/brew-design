...
| Blank brewery figures name the built-in one | A blank box in My brewery (Options tab) says nothing about what a new recipe will get; show the built-in figure greyed inside the empty box (a `placeholder` on the shared input row, read from `defaultRecipeState()` through `display.js`), as the Home/Pro and gravity-unit choices already do with "Built-in (Home)" and "Built-in (°P)" (noticed building brewery defaults, 2026-09-23) | look at it |
