...
|---|---|---|
| S1 · Housekeeping | Hooks paragraph, Recipe file in SPEC, Rule 16 and the Yeast card, Ingredient README, README (one docs item) · Starter file header · Line endings · `npm audit fix` · Coverage gaps · Dead code and One warning style · IBU shows "NaN" when blank · Title + favicon | S2 |
...
| Item | Sentence (draft) | Re-test | Session |
|---|---|---|---|
| Post-boil volume as measured | The screen and the printed sheet show the post-boil volume only at the 60 °F reference, so a brewer who measures it hot (post-boil measurement temperature above 60 °F) compares a hot reading in the sheet's measured box against a cold prediction; the selector returns the post-boil volume at its measurement temperature as well, and both surfaces show it beside the 60 °F figure (noticed building the print sheet, 2026-09-23) | suites + inspector + far end | S2 |
| Saved rows checked inside | The reader checks only a saved recipe's top-level fields, so a hand-damaged file or saved copy whose malt, hop or yeast lacks its name loads and then breaks the name box it feeds (the malt and hop rows since the searchable boxes; the yeast since the Yeast card); check the shape inside the rows and the yeast, and refuse such a document like any other damaged one (noticed building the Yeast card, 2026-09-23) | suites + inspector | S2 |
...
| Empty-field handling | Clearing a field explains why downstream stats are blank and never writes NaN into state | suites + inspector | S2 |
...
| README | Describes the app as it is now, not "later phases" | none | S1 |
