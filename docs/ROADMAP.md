...
| Searchable malt and hop boxes | The malt, boil-hop and dry-hop name boxes search the owner's ingredient list as the brewer types, each suggestion showing its numbers; picking copies FGDB and colour, or alpha, into the row; every number stays editable, a name not on the list can still be typed, and typing alone changes no number. No saved-format change. Scope table agreed: `docs/items/searchable-malt-hop-boxes.md` | suites + inspector + far end |
| Yeast card | A Yeast card between Grist and Hops: the strain searched from the list by name, lab or product code; its lab range and attenuation shown as information; the brewer types their own attenuation (moved from Grist) and fermentation temperature; picking sets ale/lager; a warning when the temperature is outside the lab range; saved format version 4. The old card becomes Pitch & Starter. After the searchable malt and hop boxes. Scope table agreed: `docs/items/yeast-card.md` | suites + inspector + far end |
| Design warnings (A + B) | Mash Rv outside 1.25–2 qt/lb and mash R outside 2.5–4 lb/lb warn beside their values (the owner's Recipe Designer, Grist and Pitch Calc's F3 and F4); a post-boil volume below the fermentation volume warns on the Volumes card. Warnings change no number. The limits are spreadsheet constants in the engine, so Tier A + B. After the Yeast card. Scope table agreed: `docs/items/design-warnings.md` | suites + inspector + far end |
