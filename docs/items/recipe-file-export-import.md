...
| E10 | Files and tier | Tier B. `apps/recipe/src/persistence.js` (the document reader/writer the file shares), `apps/recipe/src/App.jsx`, `apps/recipe/src/components/Header.jsx`, `apps/recipe/test/persistence.test.js` or a new `apps/recipe/test/recipe-file.test.js`, `SPEC.md` (rule 13 gains one sentence: a recipe file carries the same document, and importing a foreign, damaged or newer-version file is refused with the current recipe untouched — the opposite of storage's silent fallback, so it must be written down where the fallback is), `docs/TEST_COVERAGE.md`, `docs/ROADMAP.md` (this item's row removed), this file | Smallest change; persistence and App are Tier B paths |
| E11 | Sharing the reader | Storage and import read the document through one parser that says *why* a document is unreadable (not a recipe, damaged, newer version). Storage maps any reason to the defaults, as today; import maps each reason to its refusal message. The existing loader's checks — readable versions, the shape check, restoring cleared fields — are not duplicated | One reader, so a file and the saved copy can never disagree about what a valid recipe is |
